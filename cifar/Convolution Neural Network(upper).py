...
images_test,labels_test=cifar10_input.inputs(eval_data=True, #生成测试数据
                                             data_dir=data_dir,
...
dim=reshape.get_shape()[1].value                #获取特征长度
...
def loss(logits,labels):
    ...
    return tf.add_n(tf.get_collection('losses'),name='total_loss') #这里的loss包括L2loss和最后的计算loss

loss=loss(logits,label_holder)
train_op=tf.train.AdamOptimizer(1e-3).minimize(loss)#优化器

top_k_op=tf.nn.in_top_k(logits,label_holder,1)#输出top1准确率 即最准的那一类准确率

sess=tf.InteractiveSession()
tf.global_variables_initializer().run()       #初始化
tf.train.start_queue_runners()#启动数据增强线程列队，16个线程

####训练####
for step in range(max_steps):
    start_time=time.time()
    image_batch,label_batch=sess.run([images_train,labels_train])     #取出数据
    _,loss_value=sess.run([train_op,loss],     #每个batch进行一次优化然后进行一次cost的计算
       feed_dict={images_holder: image_batch,label_holder:label_batch})
    duration = time.time()-start_time

    if step%10==0:
        examples_per_sec=batch_size/duration       #每秒运行的图片个数
        sec_per_batch=float(duration)              #每个batch进行一次需要的时间
